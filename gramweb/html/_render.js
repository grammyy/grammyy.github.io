var interface = {
    ...
    drag:function(mouse) {
        interface.d3 = interface.offset.d3 + (mouse.x - interface.offset.x) / render.scale.x
    },
    ...
    onmousedown:function(mouse) {
        ...
        //[-395, -25, 125].forEach(arc => {
        //    if (render.withinArc(arc, mouse.x, mouse.y)) {
        //        interface.ready = false

        //        document.addEventListener("mousemove", interface.drag)
        //    }
        //})
    },
    ...
    onmouseup:function() {
        ...
        document.removeEventListener("mousemove", interface.drag)
    },
}
...
window.onload = function() {
    ...
    var repoPromise = new Promise((resolve, reject) => {
        http.get("https://script.google.com/macros/s/AKfycbwYXzcV_b1Db4hyCcrFZF0PRNho3KBUXkOcEhdpLIKsYeAbj9eEMwSdoSOQgdn27V2g/exec", function(git) {
            ...
            console.error(`%cRepo fetch error: ${err}`, "color: red")
